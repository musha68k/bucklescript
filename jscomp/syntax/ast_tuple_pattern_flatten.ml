...
let rec destruct_open_tuple
    ...
    : (acc * Parsetree.expression list * _) option =
...
let map_open_tuple
  ...
  | Some (qualifiers, es, attrs ) ->
    Some (List.fold_left (fun x (flag, lid,loc,attrs) ->
      {Parsetree.
...
let flattern_tuple_pattern_vb
  ...
  match destruct_open_tuple pvb_expr [] , pvb_pat.ppat_desc with
  | Some (wholes, es, tuple_attributes), Ppat_tuple xs
    when
